...
function checkUserid(){
	   ...
	   var span = document.getElementById("sp10");
	   if(useridValue == null || useridValue.trim().length == ""){
			span.style.color="red";
			span.innerHTML="输入账号不能为空";
	   }
	   else
	   {
			...
			//这里是异步运行了，当js执行到这一句话后不会等待他的返回值，而是直接往下进行，我测试出来的效果是当你js代码执行完了这里的值才返回来。
	   	    xmlHttp1.open("GET","yanzheng1.php?id="+useridValue,false);//这个页面便是你要进行选择查询的PHP页面
	        xmlHttp1.send(null);
	}
}
	function byphp()
	...
	{
		document.getElementById('sp10').innerHTML =xmlHttp1.responseText;
	}
	else
	{
		var byphp100 = "请等待";//接受PHP的返回值		
		document.getElementById('sp10').innerHTML = byphp100; //设置span里的内容
	}
	}