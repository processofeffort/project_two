  ...
  <div class="lyear-layout-container">
    ...
    <aside class="lyear-layout-sidebar">    
	 ...
	 <?php
	   ...
	   $sql="update scontent set s_cif=0 where s_reuser='".$myrow1[0]."'";
          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-body">
				  <?php 
				    include("../conn/conn.php");
				  	echo "<div id='hiti'>无任何评论回复！</div>";  
				  	$pagesize=10;
				    $sql="select username from user where name='".$_SESSION['name']."'";
				    $result1=mysqli_query($conn,$sql);
				  	$myrow1=mysqli_fetch_array($result1);
					$sqlstr="select * from scontent where s_own!=1 and s_own!=0";
					...
					while($myrow=mysqli_fetch_array($result)){
						if($myrow['s_ruser']==$myrow1[0]){												
				  ?>
				   <div class="form-group" style="border:1px solid #ccc;border-radius:5px;padding:5px;">
					<label>用户<?php echo $myrow['s_reuser'];?></label>对您的评论<label><?php echo $myrow['s_ocontent'];?></label>回复了<label><?php echo $myrow['s_content'];?></label>
					<br/>
					<label>时间:</label>
