          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-body">
				  <?php 
				    include("../conn/conn.php");
				    echo "<div id='hiti'>您还未发过任何评论，先去评论留言吧！</div>";
				  	$pagesize=10;
					$sqlstr="select * from user inner join scontent on scontent.s_tid=user.id inner join comment on scontent.s_cid=comment.mid where s_own!=1 and name='".$_SESSION['name']."'";
				  ...
				  ?>
				   <div class="form-group" style="border:1px solid #ccc;border-radius:5px;padding:5px;">
                    <label>内容:</label>
					<a href="../comment/information.php?title=<?php echo $myrow['title'];?>&&mid=<?php echo $myrow['mid'];?>&&belong=<?php echo $myrow['classes'];?>"><?php echo $myrow['s_content'];?></a>
					<a href="javasrcipt:;" onClick="showsimple(<?php echo $myrow['s_scid']?>)" style="float: right">删除</a>
					<br/>
					<label>时间:</label>
					<?php echo $myrow['s_datatime'];?>
					<br/>
					<label>来源于:</label>
