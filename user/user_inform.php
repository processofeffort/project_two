          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-body">
				  <?php 
				    include("../conn/conn.php");
				    echo "<div id='hiti'>无任何举报记录！</div>";
				  	$pagesize=10;
					$sqlstr="select * from user inner join report on report.wsuser=user.username where name='".$_SESSION['name']."'";
				  ...
				  ?>
				   <div class="form-group" style="border:1px solid #ccc;border-radius:5px;padding:5px;">
                    <label>内容:</label>
					<?php echo $myrow['wtext'];?>
					<br/>
					<label>时间:</label>
					<?php echo $myrow['wdatatime'];?>
					<br/>
					<label>举报原因:</label>
					<?php echo $myrow['wresult'];?>
					<br/>
					<label>审核状态:</label>
					<?php
					if($myrow['wstatus']=='审核中'){
							echo "<span style='color:#48b0f7'>审核中</span>";
						}else if($myrow['wstatus']=='处理成功'){
							if($myrow['wend']=='举报成功'){
								echo  "<span style='color:#27FD82'>举报成功</span>";
							}
						else{
						    echo  "<span style='color:#f96868'>举报失败</span>";
						}	
						}
						else{
